...
class AppPath(object):
  ...
  """
    ...
    def __init__(self, app_name=None, app_author=None, version=None, roaming=False, multi_path=False):
    ...
    @property
    def user_data(self):
        return self.user_data_path(self.app_name, self.app_author, version=self.version, roaming=self.roaming)

    @property
    def site_data(self):
        return self.site_data_path(
            self.app_name, self.app_author, version=self.version, multi_path=self.multi_path
        )

    @property
    def user_config(self):
        return self.user_config_path(
            self.app_name, self.app_author, version=self.version, roaming=self.roaming
        )
    ...
    @property
    def user_cache(self):
        return self.user_cache_path(self.app_name, self.app_author, version=self.version)

    @property
    def user_state(self):
        return self.user_state_path(self.app_name, self.app_author, version=self.version)

    @property
    def user_log(self):
        return self.user_log_path(self.app_name, self.app_author, version=self.version)
    ...
    """
        ...
        if SYSTEM == "win32":
            ...
            if app_name:
                if app_author is not False:
                    path = pathlib.Path.joinpath(path, app_author, app_name)
                else:
                    path = pathlib.Path.joinpath(path, app_name)
        elif SYSTEM == "darwin":
        ...
        if app_name and version:
            path = pathlib.Path.joinpath(path, version)
        return path
        ...
        if SYSTEM == "win32":
            ...
            if app_name:
                if app_author is not False:
                    ...
                    path = pathlib.Path.joinpath(path, app_name)
                if opinion:
                    path = pathlib.Path.joinpath(path, "Cache")
        elif SYSTEM == "darwin":
    ...
    """
        if SYSTEM == "darwin":
            ...
            version = False
            if opinion:
                path = pathlib.Path.joinpath(path, "Logs")
        else:
            path = AppPath.user_cache_path(app_name, app_author, version)
            version = False
            if opinion:
                path = pathlib.Path.joinpath(path, "log")
        if app_name and version:
            path = pathlib.Path.joinpath(path, version)
        return path
...
if __name__ == "__main__":
    ...
    dirs = AppPath(_app_name, _app_author, version="1.0")
    for prop in props:
